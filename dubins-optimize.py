# Libraries
import matplotlib.pyplot as plt
import numpy as np
from matplotlib.animation import FuncAnimation, PillowWriter
from scipy.stats import norm

# Define the dynamics
def dubins_discrete(state, v, umax, seg, s, dt=0.1):
    x, y, theta = state
    

# Compute Markovian transition probabilities
def compute_probabilities(params1, params2, sigma1=1.0, sigma2=1.0):
    M = len(params1) - 1
    N = len(params2) - 1
    probabilities = np.empty((M, N, M, N), dtype=float)

    # Outer loop loops over current cells (i,j)
    for i in range(M):
        for j in range(N):
            a_lo = params1[i]
            a_hi = params1[i+1]
            b_lo = params2[j]
            b_hi = params2[j+1]
            mid_x = [0.5 * (a_lo + a_hi), 0.5 * (b_lo + b_hi)]
            mu = dynamics(np.array(mid_x))

            # Inner loop loops over target cells (k,l)
            for m in range(M):
                for n in range(N):
                    Am = norm.cdf(params1[m+1], loc=mu[0], scale=sigma1) - norm.cdf(params1[m], loc=mu[0], scale=sigma1)
                    Bn = norm.cdf(params2[n+1], loc=mu[1], scale=sigma2) - norm.cdf(params2[n], loc=mu[1], scale=sigma2)
                    probabilities[i, j, m, n] = Am * Bn
    return probabilities

def fail_within_N_steps(T, s0, failure_index=None, N=50):
    n = T.shape[0]
    if failure_index is None:
        failure_index = n - 1

    non_fail = [i for i in range(n) if i != failure_index]
    Q = T[np.ix_(non_fail, non_fail)]
    r = T[non_fail, failure_index]

    # f_k[i] = P(fail by time k starting from non_fail state i)
    f = np.zeros(len(non_fail))

    for _ in range(N):
        # new_f = r + Q @ f
        f = r + Q @ f

    # build full vector
    x = np.zeros(n)
    x[non_fail] = f
    x[failure_index] = 1.0
    return float(x[s0])


def failure_reach_probability(P, s0, failure_index=None):

    n = P.shape[0]
    if failure_index is None:
        failure_index = n - 1  # default: last state
    
    # quick sanity checks (optional but helpful)
    assert P.shape == (n, n)
    # failure state should be absorbing: P[f, f] ~ 1, others ~ 0
    if not np.allclose(P[failure_index], np.eye(n)[failure_index]):
        print("Warning: failure state does not look absorbing.")
    
    # indices of non-failure states
    non_fail = [i for i in range(n) if i != failure_index]
    m = len(non_fail)  # should be n-1
    
    # Submatrix Q: transitions among non-failure states
    Q = P[np.ix_(non_fail, non_fail)]        # shape (m, m)
    # Vector r: transitions from non-failure to failure
    r = P[non_fail, failure_index]           # shape (m,)
    
    # Solve (I - Q) x = r for x
    I = np.eye(m)
    A = I - Q
    x_nonfail = np.linalg.solve(A, r)
    
    # Build full vector of hitting probabilities
    x = np.zeros(n)
    x[non_fail] = x_nonfail
    x[failure_index] = 1.0
    
    return float(x[s0])

def objective_function(params1, params2):
    probabilities = compute_probabilities(params1, params2, sigma1=0.5, sigma2=0.5)

    # Convert 4D array to 2D transition matrix
    M = len(params1) - 1
    N = len(params2) - 1
    num_states = M * N
    T_part = probabilities.reshape(num_states, num_states)

    # Add absorbing/failure states
    row_sums = T_part.sum(axis=1)
    missing_mass = 1.0 - row_sums
    T = np.zeros((num_states + 1, num_states + 1))
    T[:num_states, :num_states] = T_part
    T[:num_states, -1] = missing_mass
    T[-1, -1] = 1.0
    T = np.array(T)

    cost = 0.0
    for s in range(num_states):
        cost += fail_within_N_steps(T, s0=s, N=100)
    cost /= num_states

    return cost

def gradient_objective_function(params1, params2):
    """
    Compute dJ/dparams1, dJ/dparams2 where

    J = P(fail within N steps | initial abstract state is chosen
          uniformly at random over all grid cells).

    params1, params2 : lists/arrays of grid boundaries (length M+1, N+1)

    Returns
    -------
    grad_cost_1 : np.ndarray, same shape as params1
    grad_cost_2 : np.ndarray, same shape as params2
    """
    import numpy as np
    from scipy.stats import norm

    # --- Settings (keep consistent with your abstraction) ---
    sigma1 = 0.5
    sigma2 = 0.5
    horizon = 100  # time horizon N
    A_dyn = np.array([[0.8, -0.3],
                      [0.3,  0.8]])  # must match `dynamics`

    params1 = np.asarray(params1, dtype=float)
    params2 = np.asarray(params2, dtype=float)

    M = len(params1) - 1  # #cells in x1
    N = len(params2) - 1  # #cells in x2
    num_states = M * N
    m = num_states        # number of non-failure states

    # ------------------------------------------------------------------
    # 1) Build Markov chain on grid: Q (safe->safe), r (safe->failure)
    # ------------------------------------------------------------------
    probabilities = compute_probabilities(params1, params2,
                                          sigma1=sigma1, sigma2=sigma2)
    T_part = probabilities.reshape(num_states, num_states)  # safe-to-safe
    row_sums = T_part.sum(axis=1)
    Q = T_part.copy()
    r = 1.0 - row_sums  # safe-to-failure

    # ------------------------------------------------------------------
    # 2) Backward DP: f_hist[k][i] = P(fail within k steps | start at i)
    # ------------------------------------------------------------------
    f_hist = [np.zeros(m)]
    for _ in range(horizon):
        f_new = r + Q @ f_hist[-1]
        f_hist.append(f_new)
    # f_hist has length horizon+1, f_hist[k] = f_k

    # ------------------------------------------------------------------
    # 3) Forward occupancy: u_hist[t][i] = P(X_t=i, no fail up to t)
    #    with initial state distributed uniformly over all cells.
    # ------------------------------------------------------------------
    u_hist = []
    u0 = np.full(m, 1.0 / m)   # uniform over all non-failure states
    u_hist.append(u0)
    for t in range(1, horizon):
        u_hist.append(u_hist[-1] @ Q)

    # ------------------------------------------------------------------
    # 4) Gradient wrt Q and r for J = P_fail under uniform initial dist
    #    dJ/dQ[i,j] = sum_t u_t[i] * f_{N-1-t}(j)
    #    dJ/dr[i]   = sum_t u_t[i]
    # ------------------------------------------------------------------
    dJ_dQ = np.zeros_like(Q)
    dJ_dr = np.zeros_like(r)

    for t in range(horizon):
        dJ_dQ += np.outer(u_hist[t], f_hist[horizon - 1 - t])
        dJ_dr += u_hist[t]

    # Effective weight for each safe->safe transition T_part[i,j]
    # because r[i] = 1 - sum_j T_part[i,j]
    # => dJ/dT_part[i,j] = dJ/dQ[i,j] - dJ/dr[i]
    W_2D = dJ_dQ - dJ_dr[:, None]          # shape (m, m)
    weights = W_2D.reshape(M, N, M, N)     # reshape to [i,j,m,n]

    # ------------------------------------------------------------------
    # 5) Gradient wrt params1 (x1 boundaries) and params2 (x2 boundaries)
    # ------------------------------------------------------------------
    grad_cost_1 = np.zeros_like(params1)
    grad_cost_2 = np.zeros_like(params2)

    for i in range(M):
        a_lo = params1[i]
        a_hi = params1[i + 1]
        mid1 = 0.5 * (a_lo + a_hi)

        for j in range(N):
            b_lo = params2[j]
            b_hi = params2[j + 1]
            mid2 = 0.5 * (b_lo + b_hi)

            mid_x = np.array([mid1, mid2])
            mu = dynamics(mid_x)
            mu1, mu2 = mu

            for m_idx in range(M):
                a_m = params1[m_idx]
                a_m1 = params1[m_idx + 1]

                z_lo = (a_m - mu1) / sigma1
                z_hi = (a_m1 - mu1) / sigma1
                Phi_hi1 = norm.cdf(z_hi)
                Phi_lo1 = norm.cdf(z_lo)
                phi_hi1 = norm.pdf(z_hi)
                phi_lo1 = norm.pdf(z_lo)

                A_m = Phi_hi1 - Phi_lo1
                dA_dmu1 = (phi_lo1 - phi_hi1) / sigma1

                for n_idx in range(N):
                    w = weights[i, j, m_idx, n_idx]
                    if w == 0.0:
                        continue

                    b_n = params2[n_idx]
                    b_n1 = params2[n_idx + 1]

                    v_lo = (b_n - mu2) / sigma2
                    v_hi = (b_n1 - mu2) / sigma2
                    Phi_hi2 = norm.cdf(v_hi)
                    Phi_lo2 = norm.cdf(v_lo)
                    phi_hi2 = norm.pdf(v_hi)
                    phi_lo2 = norm.pdf(v_lo)

                    B_n = Phi_hi2 - Phi_lo2
                    dB_dmu2 = (phi_lo2 - phi_hi2) / sigma2

                    # ---------- gradient wrt params1 (a_k) ----------
                    # Only indices that matter: a_i,a_{i+1} (cell midpoint)
                    # and a_m,a_{m+1} (target interval).
                    for k in {i, i + 1, m_idx, m_idx + 1}:
                        if k < 0 or k >= M + 1:
                            continue

                        # d(mid1)/d a_k
                        dmid1_da = 0.5 if (k == i or k == i + 1) else 0.0
                        # d mu / d a_k via mid1
                        dmu1_da = A_dyn[0, 0] * dmid1_da
                        dmu2_da = A_dyn[1, 0] * dmid1_da

                        # A_m: via mu1 and bounds a_m,a_m1
                        dA_da = dA_dmu1 * dmu1_da
                        if k == m_idx:
                            dA_da += (-phi_lo1) / sigma1  # lower bound
                        if k == m_idx + 1:
                            dA_da += (phi_hi1) / sigma1   # upper bound

                        # B_n: via mu2 only (b_n,b_{n+1} are params2)
                        dB_da = dB_dmu2 * dmu2_da

                        dp_da = dA_da * B_n + A_m * dB_da
                        grad_cost_1[k] += w * dp_da

                    # ---------- gradient wrt params2 (b_l) ----------
                    # Only b_j,b_{j+1} (cell midpoint) and b_n,b_{n+1} (interval)
                    for l in {j, j + 1, n_idx, n_idx + 1}:
                        if l < 0 or l >= N + 1:
                            continue

                        dmid2_db = 0.5 if (l == j or l == j + 1) else 0.0
                        dmu1_db = A_dyn[0, 1] * dmid2_db
                        dmu2_db = A_dyn[1, 1] * dmid2_db

                        # A_m: via mu1 (params1 hold the a-bounds)
                        dA_db = dA_dmu1 * dmu1_db

                        # B_n: via mu2 and bounds b_n,b_{n+1}
                        dB_db = dB_dmu2 * dmu2_db
                        if l == n_idx:
                            dB_db += (-phi_lo2) / sigma2  # lower bound
                        if l == n_idx + 1:
                            dB_db += (phi_hi2) / sigma2   # upper bound

                        dp_db = dA_db * B_n + A_m * dB_db
                        grad_cost_2[l] += w * dp_db

    return grad_cost_1, grad_cost_2



# Gradient descent optimization
def gradient_descent(params1, params2, learning_rate=0.1, max_iters=5000, tol=1e-3, cushion = [1e-2, 1e-3], verbose=True):
    # Make a copy so we don't overwrite the original list
    params1 = params1[:]
    params2 = params2[:]
    history = []
    params1_history = [params1[:]]  # Track parameter evolution
    params2_history = [params2[:]]

    for it in range(max_iters):
        cost = objective_function(params1, params2)
        
        # Check for NaN - early termination if unstable
        if np.isnan(cost):
            print(f"WARNING: NaN encountered at iteration {it}. Stopping.")
            print("Try reducing the learning rate.")
            break
            
        history.append(cost)

        if verbose and it % 1 == 0:
            print(f"Iter {it:5d} | cost = {cost:.4e}")

        # Stopping criterion: cost not changing much
        grad1, grad2 = gradient_objective_function(params1, params2)
        grad_norm = np.sqrt(np.sum(np.array(grad1)**2) + np.sum(np.array(grad2)**2))
        if grad_norm < tol:
            if verbose:
                print(f"Converged at iter {it} with cost {cost:.6e}")
            break

        grad = gradient_objective_function(params1, params2)

        # Update only interior parameters (keep endpoints fixed)
        for k in range(1, len(params1) - 1):
            params1[k] -= learning_rate * grad[0][k]
            # Clamp to stay within bounds and maintain ordering
            params1[k] = np.clip(params1[k], params1[k-1] + cushion[0], params1[k+1] - cushion[0])
            
        for k in range(1, len(params2) - 1):
            params2[k] -= learning_rate * grad[1][k]
            # Clamp to stay within bounds and maintain ordering
            params2[k] = np.clip(params2[k], params2[k-1] + cushion[1], params2[k+1] - cushion[1])
            
        params1_history.append(params1[:])
        params2_history.append(params2[:])

    return params1, params2, history, params1_history, params2_history

x1min, x1max = -10, 10
x2min, x2max = -10, 10
# params1 = np.linspace(x1min, x1max, 11).tolist()
# params2 = np.linspace(x2min, x2max, 11).tolist()
# Generate random interior points and include endpoints
num_interior_points = 8
interior1 = np.random.uniform(x1min, x1max, num_interior_points)
interior2 = np.random.uniform(x2min, x2max, num_interior_points)
params1 = sorted([x1min] + interior1.tolist() + [x1max])
params2 = sorted([x2min] + interior2.tolist() + [x2max])


