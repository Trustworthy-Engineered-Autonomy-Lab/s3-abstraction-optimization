#!/usr/bin/env python3
"""Run synthetic-v3 model checking and recall on saved CEGAR models."""
...
from run_synthetic import load_model_checkpoint
...
def evaluate_checkpoint(checkpoint: Path, gt_path: Path, write: bool) -> dict:
    ...
    recall = compute_synthetic_v3_recall(
        absys,
        satisfying_initial,
        gt_reach_regions,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Load saved synthetic CEGAR abstractions, run pyModelChecking "
            "with synthetic-v3's property, time it, and compute v3 recall."
        )
    )
